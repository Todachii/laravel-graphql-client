      ...
      <v-layout row wrap>
        ...
        <v-flex xs12>
          <v-sheet class="pa-4" elevation="6">
            <v-text-field v-model="account.name" label="name" required></v-text-field>
            <v-text-field v-model="account.twitter_id" label="twitter_id" disabled></v-text-field>
            <v-flex>
              <img
...
export default {
  ...
  apollo: {
    account: {
      // query2: variablesを入れて、返却データを変更するパターン
      query: ACCOUNT,
      loadingKey: "loading",
      update(data) {
        return data.Account;
